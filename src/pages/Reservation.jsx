        ...
        <div className="flex flex-col md:flex-row">
          ...
          <div className="p-4 flex-1">
            <div className="flex items-start justify-between mb-2">
              ...
              <Badge variant={reservation.status === 'upcoming' ? 'default' : 'secondary'}>
                {reservation.status}
              </Badge>
            </div>
        ...
        {reservations.length === 0 ? (
          ...
          <Tabs defaultValue="upcoming">
            ...
            <TabsContent value="upcoming" className="space-y-4 mt-6">
              {upcoming.map(res => <ReservationCard key={res.id} reservation={res} />)}
              {upcoming.length === 0 && (
                <p className="text-muted-foreground text-center py-8">No upcoming reservations</p>
              )}
            </TabsContent>
            <TabsContent value="past" className="space-y-4 mt-6">
              {past.map(res => <ReservationCard key={res.id} reservation={res} />)}
              {past.length === 0 && (
